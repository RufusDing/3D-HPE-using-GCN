...
class _ResGraphConv(nn.Module):
    def __init__(self, convm, adj, input_dim, output_dim, hid_dim, p_dropout):
        super(_ResGraphConv, self).__init__()

        self.gconv1 = _GraphConv(convm, adj, input_dim, hid_dim, p_dropout)
        self.gconv2 = _GraphConv(convm, adj, hid_dim, output_dim, p_dropout)
...
class SemGCN(nn.Module):
    def __init__(self, convm, adj, hid_dim, coords_dim=(2, 3), num_layers=4, nodes_group=None, p_dropout=None):
        super(SemGCN, self).__init__()

        _gconv_input = [_GraphConv(convm, adj, coords_dim[0], hid_dim, p_dropout=p_dropout)]
        _gconv_layers = []

        if nodes_group is None:
            for i in range(num_layers):
                _gconv_layers.append(_ResGraphConv(convm, adj, hid_dim, hid_dim, hid_dim, p_dropout=p_dropout))
        else:
            group_size = len(nodes_group[0])
            assert group_size > 1

            grouped_order = list(reduce(lambda x, y: x + y, nodes_group))
            restored_order = [0] * len(grouped_order);print(grouped_order)
        ...
        self.gconv_output = SemGraphConv(convm, hid_dim, coords_dim[1], adj)
